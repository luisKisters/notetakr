...
Running list of product ideas. Bigger explorations get their own file in
`design/ideas/` (e.g. `participants-crm.md`).

- **Global shortcut to start transcribing** — a hotkey that immediately starts
  transcription, without first opening the floating window or touching the UI.
  Complements the existing window-toggle hotkey (⌃⌥⌘N): one shortcut to show
  the note, one to just start capturing.
- **[CRM-linked participants](design/ideas/participants-crm.md)** (parked) —
  participants as pickable people linked to a CRM/people index instead of plain
  strings, with hover cards and per-person meeting history.

---

# Redesign — Design Ideas & Open Questions

> **STATUS: DECIDED.** The final locked design lives in `design/mockups/final-editor.html`,
> `design/mockups/final-switcher.html`, `design/mockups/final-settings.html` (round 5).
> Everything below is the exploration history. Locked spec summary:
>
...
Mockups (5 variations each, in `design/mockups/`, open in a browser):
...
- [ ] Participants as plain strings now; CRM-linked objects later (see `design/ideas/participants-crm.md`).
